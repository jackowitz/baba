...
the latest and greatest at the time. As the song took on its anthemic status
many tried to reproduce the effect but consistently fell short, even using the
supposed same equipment. It was only years later that the truth surfaced - the
most famous synthesizer in music wasn't a synthesizer at all! It was in fact a
Lowrey TBO-1 home organ, a bottom-of-the-line household model, but with an
unconventional setting that left it's mark on a generation. The following is
an analysis of exactly what this setting is and why it is so difficult to
replicate using traditional instruments, concluding with an implementation
modeling the effect using the Euterpea Haskell library.
